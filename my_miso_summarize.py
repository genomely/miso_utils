        ...
        if os.path.split(root)[1][:3] == 'chr':
            for f in files:
                if f[-7:] == '.pickle':
                    ...
                    sampleName = re.search("([A-Z]{4}_RNA_[A-Z]+_\d+)", root).group(0)
...
def summarizeGeneMisoFile_listener(q):
    while 1:
        ...
        global sampleGeneSummary
        global sampleIsoformSummary
        ...
        print('\rProcessed {0} genes from sample {1}'.format(len(sampleGeneSummary), sampleName))
...
def outputPsiMatrix(filePrefix, psiObj, sampleList):
    '''
    Output psis matrix
    '''
                                        
    outputFileName = filePrefix + '_psiMatrix.txt' 
    ...
    for isoID, isoObj in psiObj.iteritems():
...
def main():
    ...
    for sampleName, misoPaths in sample_gene_miso_files.iteritems():
            job = pool.apply_async(summarizeGeneMisoFiles_worker, (sampleName, misoPaths, q))
            jobs.append(job)
    ...
    outputPsiMatrix(outputFilePrefix, sampleIsoformSummary, sampleList)
    
    '''
    hopefully output of matrices shouldn't be any different!
    '''
